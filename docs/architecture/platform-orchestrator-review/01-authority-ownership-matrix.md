...
| --- | --- | --- | --- | --- | --- |
| `CONFIRMED` | Customer organization lifecycle | Platform Customer Ownership | Customer Ownership use case | Customer Ownership store | Platform ADR-0007 confirms strategic ownership and terminal lifecycle; tactical aggregates remain proposed |
| `CONFIRMED` | Tenant isolation and lifecycle | Platform Tenancy or Standalone Authority | Owning authority use case | Owning authority store | Platform ADR-0007 confirms Platform Tenancy ownership and lifecycle semantics; Standalone remains an alternative external authority |
...
| `CONFIRMED` | Stable Platform principal and external identity lifecycle | Platform Identity | Identity registration, binding, migration, disablement, erasure, and tombstone use cases | Platform identity store | Platform ADR-0007 confirms ownership, non-reuse, proof-based rebinding, and privacy semantics; tactical aggregates remain proposed |
| `CONFIRMED` | Membership, grant, one-level delegation, revocation, and product authority | Platform Access and Authority | Membership, grant, delegation, revocation, and capability-decision use cases | Platform product-authority store | Platform ADR-0007 confirms explicit Tenant grants and bounded delegation; exact aggregates and decision API remain proposed |
| `CONFIRMED` | Commercial entitlements and restrictions | Platform Commercial Access | Platform entitlement use case | Platform commercial store | Platform ADR-0007 confirms the narrow commercial boundary and fail-closed expiry semantics; tactical aggregates remain proposed |
...
- `CONFIRMED`: Platform ADR-0007 accepts all seven Platform strategic context
  owners and PO-PLAT-001 through PO-PLAT-007. Only Project Management is
  authorized for package materialization; this does not accept cross-repository
  APIs or tactical packages for the other six contexts.
...
- `OPEN`: exact aggregate boundaries and first slices inside Platform Identity,
  Access and Authority, and Tenancy. Project Management's first slice is
  accepted by ADR-0007; later slices remain separately gated.
