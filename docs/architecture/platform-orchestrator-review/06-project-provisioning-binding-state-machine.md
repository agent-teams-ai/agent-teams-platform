...
| --- | --- | --- | --- | --- | --- |
| `CONFIRMED` | ProductProject | Platform Project Management | `OPEN` or terminal `RETIRED`, lifecycle revision, retirement epoch | Retirement commit CAS, receipt, and outbox are atomic; Project-create receipt/process UoW remains proposed | Platform ADR-0004 confirms retirement atomicity only |
...
| `PROPOSED` | Managed scope-admission process | Platform Project Management | Process identity, immutable request digest, and bounded step obligations with command and receipt refs | Eventual convergence; unknown steps queried by original stable identity | Proposed Project Management ownership; exact aggregate, cancellation, and blocked semantics remain open |
