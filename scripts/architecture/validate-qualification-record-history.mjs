...
const recordPath = "architecture/deployment-profiles/qualification-records";
...
export function validateQualificationHistoryEntries(entries) {
  ...
  for (const entry of entries) {
    ...
    errors.push(
      `DEPLOY-RECORD-IMMUTABLE-001 ${paths.join(" -> ")}: published qualification records are append-only; Git status ${status} is forbidden`,
    );
  }
  return errors;
}
...
async function main() {
  ...
  console.log("Qualification record history valid: append-only policy preserved.");
}
